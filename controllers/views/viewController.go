...
func (c *ViewController) Home() {
	...
	c.UsePartArticleListData("最新发布", pArticleListData, page, limit, count, "/list/page/")
	...
	c.EchoLayoutTpl("home", LAYOUT_OPTION_STYLE)
}
...
func (c *ViewController) Preview() {
	...
	if err == nil {
		...
		case 1:
			...
			c.EchoLayoutTpl("content_md_preview")
		case 2:
			// 富文本
			// c.TplName = "index/content_wang_editor_preview.html"
			c.EchoLayoutTpl("content_wang_editor_preview")
...
func (c *ViewController) Content() {
	...
	c.EchoLayoutTpl(TplName, LAYOUT_OPTION_STYLE)
}
...
func (c *ViewController) AnthologyHome() {
	...
	for _, v := range articleList {
		latest_html_label := false
		if time.Now().Unix()-v.ReleaseUpdateTime.Unix() < 432000 {
			latest_html_label = true
		}
		// 获取标签
		tags := []TagItem{}
	...
	c.EchoLayoutTpl("anthology_home", LAYOUT_OPTION_STYLE)
}
...
func (c *ViewController) SearchTag() {
	...
	if err != nil || tagInfo.ID == 0 {
		...
		c.UsePartArticleListData("共 "+strconv.FormatInt(articleCount, 10)+" 篇文章", articleListItem, pageInt, 15, articleCount, searchUrl)
		c.EchoLayoutTpl("search")
	}
...
// 专栏列表
func (c *ViewController) AnthologyList() {
	{
		global_seo := cache.ConfigCacheGroupGet("global_seo")
		global_site := cache.ConfigCacheGroupGet("global_site")
		// ==== 头板块数据 ====
		background_image, _ := global_site["background_image"].(string)
		c.UsePartHeaderData(
			"专栏 - "+cmn.InterfaceToString(global_site["title"]),
			cmn.InterfaceToString(global_seo["site_description"]),
			cmn.InterfaceToString(global_seo["site_keywords"]),
			cmn.InterfaceToString("/"+background_image),
			cmn.InterfaceToString(global_site["about_url"]),
		)
		c.SetPartHeaderMenuCheck("anthologys")
	...
	mAnthology := models.Anthology{}
	// 查询所有已经开放的分类
	anthologyList, _ := mAnthology.GetList(cmn.Msi{
		"golbal_open": 1,
	})

	anthologyData := cmn.Msi{
		"list":  anthologyList,
		"count": 100,
	}
	c.Data["AnthologyData"] = anthologyData

	c.EchoLayoutTpl("anthology_list", LAYOUT_OPTION_STYLE)
}
