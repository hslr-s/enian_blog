layui.define(["form"], function(exports) {
	...
	tree.prototype.template = function(data) {
		...
		o.each(data, function(index, item) {
			var checked = t.c.is_checkbox && t.c.checked && o.inArray(item.id, t.c.checked) > -1 && 'checked',
				hide_class = 'class="' + (item.pid == 0 || item.pid == t.cache(item.pid) || t.c.is_open ? "" : "layui-hide") + '"',
	...
	tree.prototype.render = function(e) {
		...
		o(t.c.elem).html(template).on("click", "td", function() {
			...
			if(dt.length) {
				if(status) {
					...
					o(t.c.elem).find("tr[data-pid=" + id + "]").removeClass('layui-hide');
					t.cache(id, true);
		...
		o(t.c.elem).find('[lay-filter=lay-t]').next().css('padding-left','0').parents('td').css('text-align','center');
	};
